...
export default function Home() {
  ...
  const [sortedIMTLeaves, setSortedIMTLeaves] = useState<number>(100)
  const [smtTimes, setSMTTimes] = useState<number[]>([])
  const [leanIMTTimes, setLeanIMTTimes] = useState<number[]>([])
  const [sortedIMTTimes, setSortedIMTTimes] = useState<number[]>([])
  ...
  const runLeanIMTFunctions = useCallback(async () => {
    ...
    await leanIMT.insertMany(members)
  ...
  const runSortedIMTFunctions = useCallback(async () => {
    const timeValues = []

    const { commitment: commitment0 } = new Identity()

    const sortedIMTHash = (a: bigint, b: bigint) => poseidon2([a, b])
    const sortedIMT = new SortedIMT(sortedIMTHash)

    const members = Array.from({ length: sortedIMTLeaves - 1 }, (_, i) =>
      BigInt(i + 1)
    )
    ...
    for (let i = 0; i < members.length; i++) {
      sortedIMT.insert(members[i])
    }

    const [proof, time0] = await run(() =>
      sortedIMT.generateProof(commitment0)
    )

    timeValues.push(time0)
    setSortedIMTTimes(timeValues.slice())

    const [, time1] = await run(() => sortedIMT.verifyProof(proof))
    ...
    const [, time2] = await run(() => sortedIMT.insert(commitment1))
    ...
    <div className="flex flex-col my-10 mx-10">
      <div className="flex flex-wrap gap-y-20 justify-around w-full">
        ...
        <div className="flex flex-col gap-6 justify-end items-start">
          <div className="text-2xl font-bold mb-14">SortedIMT</div>
          <div className="flex flex-col gap-4">
            <InputNumber
              title="Tree Leaves"
              defaultValue={100}
              onChange={setSortedIMTLeaves}
            />
            <button
              onClick={runSortedIMTFunctions}
                ...
                <div key={i} className="flex items-center gap-6">
                  <div className="flex gap-6 py-2">
                    ...
                    <div className="font-normal">
                      {sortedIMTTimes[i]
                        ? `${prettyMilliseconds(sortedIMTTimes[i], { millisecondsDecimalDigits: 1 })}`
                        : "0ms"}
                    </div>
                  </div>
                </div>
